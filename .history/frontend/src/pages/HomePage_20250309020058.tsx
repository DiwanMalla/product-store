...
const HomePage = () => {
  ...
  return (
    <section className="h-screen flex justify-center pt-15 h-screen bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {products.map((product) => (
                <ProductCard
                  ...
                  product={product}
                  onEdit={() => handleEdit(product.name)} // Pass onEdit handler
                  onDelete={() => handleDelete(product.name)} // Pass onDelete handler
                />
              ))}
            </div>
          </div>
